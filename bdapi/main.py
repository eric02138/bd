...
class RobotEvent(BaseModel):
    ...
    status: Optional[str] = None  # could be an enum, and could even be its own collection
...
async def get_export_robot_event_by_username(
    ...
    return Response(
        content=csv_content,
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=robot_events_{username}.csv"}
    )
